...
filepath=/tmp/snap-1380682;
if [ ! -f "$filepath" ]; then
  echo "Downloading latest snapshot from Nebra"
  wget -q --show-progress https://snapshots-wtf.sensecapmx.cloud/snap-1380682 -O /tmp/snap-1380682
else
...
echo "Start loading snap-1380682 at `date +%H:%M`. This can take up to 60 minutes"
sudo rm -f /home/pi/hnt/miner/snap/snap-*
sudo cp /tmp/snap-1380682 /home/pi/hnt/miner/snap/snap-1380682
> /tmp/load_result
now=`date +%s`
((sudo docker exec $minername sh -c "export RELX_RPC_TIMEOUT=3600; miner snapshot load /var/data/snap/snap-1380682" > /tmp/load_result) > /dev/null 2>&1 &)
...
do
    ...
    if [ "$result" = "ok" ]; then
       ...
       sudo rm -f /home/pi/hnt/miner/snap/snap-1380682
       rm /tmp/load_result
